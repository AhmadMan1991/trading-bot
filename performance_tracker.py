    ...
    for ts, row in df.iterrows():
        ...
        if direction in ("LONG", "BUY"):
            ...
            if h >= tp1 and not hit_tp1:
                hit_tp1 = True
            if hit_tp1 and tp2 and h >= tp2:
                return {**sig, "outcome":"TP2", "outcome_r":2.0*(tp2-entry)/risk, "resolved_at":str(ts)}
            ...
            if l <= tp1 and not hit_tp1:
                hit_tp1 = True
            if hit_tp1 and tp2 and l <= tp2:
                return {**sig, "outcome":"TP2", "outcome_r":2.0*(entry-tp2)/risk, "resolved_at":str(ts)}
...
def performance_summary() -> dict:
    ...
    if not records:
        return {}
    wins      = [r for r in records if r.get("outcome") in ("TP1","TP2")]
